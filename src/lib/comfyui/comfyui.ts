...
export async function uploadImageToComfy(file: File): Promise<{ name: string; subfolder: string }> {
...
export async function upscaleImage(
	...
	try {
		...
		onProgress?.('Uploading image...', 0);
		const { name } = await uploadImageToComfy(file);

		// Create the workflow
		const workflow = createUpscaleWorkflow(name, resolution);

		// Queue the prompt via proxy
		onProgress?.('Starting upscale...', 5);
		...
		ws.onmessage = (event) => {
			try {
				...
				if (message.type === 'progress') {
					...
					onProgress?.(`Processing... ${percent}%`, percent);
				}
				...
				if (message.type === 'execution_error') {
					console.error('[ComfyUI] Execution error:', message.data);
					clearTimeout(timeout);
					ws.close();
...
async function fetchOutputImage(promptId: string): Promise<string> {
	...
	for (let attempt = 0; attempt < maxRetries; attempt++) {
		try {
			...
			if (!response.ok) {
				console.warn(`[ComfyUI] History fetch attempt ${attempt + 1} failed:`, response.status);
				continue;
			}
...
// Get image as blob URL for display - uses proxy for ComfyUI URLs
export async function getImageBlobUrl(url: string): Promise<string> {
	// If it's a direct ComfyUI URL, convert to proxy URL
	let fetchUrl = url;
	if (url.includes('comfyui.') && url.includes('/view')) {
